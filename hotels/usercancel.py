...
""")
r="""select * from booking where Username='%s' and status="Cancel" """%(name)
cur.execute(r)
rec=cur.fetchall()
for c in rec:
    print("""
    ...
    <td><input type="text" name="status" value="%s"   style="border:none;width:60px;"></td>
    </tr>
    </form>
    """ %(c[0],c[3],c[2],c[4],c[5],c[6],c[1],c[10],c[8]))