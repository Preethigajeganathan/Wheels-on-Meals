...
""")
r = """select * from booking where Ownername='%s' and status="New" and booktype="Table Booking" """ % (name)
cur.execute(r)
rec = cur.fetchall()
for c in rec:
    ...
    <td><input type="submit" class="btn btn-info" name="sub" value="Accept"></td>
    </tr>
    </form>
    """ % (c[0], c[1], c[2], c[4], c[5], c[6],formatted_date, c[3], c[9]))
bookid = m.getvalue("bid")
usermailid = m.getvalue("usermail")
hotel = m.getvalue("hname")
submit = m.getvalue("sub")
if submit != None:
    sr = """Update booking set status="Accept" where id='%s'""" % (bookid)
    cur.execute(sr)
    con.commit()
    ...
    body = "The Table is Conformed by: {}".format(hotel)
    msg = """Subject: {} \n\n{}""".format(subject, body)
    ...
    print("""
        <script>
        alert("Table Conformed ");
        location.href="own_table.confirm.py?id=%s"
        </script>
         """%(b))
